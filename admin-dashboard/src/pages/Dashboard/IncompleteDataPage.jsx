...
import { trStyle, tdStyle, tdBoldStyle, badgeStyle } from '../styles/dashboardStyles';

const IncompleteDataPage = ({ medicines, onEdit }) => {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredIncomplete = useMemo(() => {
    const term = searchTerm.toLowerCase().trim();
    if (!term) return medicines;
    return medicines.filter(m => (m.name || m.medicineName || "").toLowerCase().includes(term));
    ...
    <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px', alignItems: 'center' }}>
        ...
        <input
          ...
          style={{ padding: '10px 15px', borderRadius: '10px', border: '1px solid #e2e8f0', width: '300px', fontSize: '14px' }}
        />
      </div>
      <TableLayout headers={['#', 'Medicine Name', 'Missing Fields', 'Actions']}>
        {filteredIncomplete.map((m, index) => {
          const missing = [];
...
export default IncompleteDataPage;
